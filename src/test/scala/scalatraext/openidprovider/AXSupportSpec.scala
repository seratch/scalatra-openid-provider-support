...
class OPAXServlet extends OPServlet with AXSupport {
...
class OPAXServletImmediateModeEnabled extends OPAXServlet {

  protected override lazy val isImmediateModeAllowed = true

}

class AXSupportSpec extends ScalatraFlatSpec with ShouldMatchers with Logging {

  addServlet(classOf[OPAXServlet], "/*")

  behavior of "AXSupport"
  ...
  it should "respond checkid_setup when being confirmed" in {
    ...
    post(chechIdWithAXPath + "&loggedin=true&confirmation=true") {
      status should equal(302)
      logger.info("Location: " + header("Location"))
      header("Location") should startWith("http://localhost:3000/consumer/complete?")
      header("Location") should include("openid.mode=id_res")
    }
    put(chechIdWithAXPath + "&loggedin=true&confirmation=true") {
      status should equal(405)
    }
  }

  it should "respond checkid_setup when being cancelled" in {
    get(chechIdWithAXPath + "&loggedin=true&confirmation=false") {
      status should equal(302)
      logger.info("Location: " + header("Location"))
      header("Location") should startWith("http://localhost:3000/consumer/complete?")
      header("Location") should include("openid.mode=cancel")
    }
    post(chechIdWithAXPath + "&loggedin=true&confirmation=false") {
      status should equal(302)
      logger.info("Location: " + header("Location"))
      header("Location") should startWith("http://localhost:3000/consumer/complete?")
      header("Location") should include("openid.mode=cancel")
    }
    put(chechIdWithAXPath + "&loggedin=true&confirmation=false") {
      status should equal(405)
    }
  }
